        ...
        <aside className="w-16 md:w-64 flex-shrink-0 border-r border-border bg-sidebar flex flex-col justify-between transition-all duration-300">
          <div className="p-4 space-y-6">
            <div className="flex items-center space-x-2 px-2">
              ...
              <span className="hidden md:inline font-bold text-lg tracking-tight uppercase text-sidebar-foreground">
                XANDA
              </span>
            </div>
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={`flex items-center px-2 py-2.5 rounded-md transition-colors ${isActive ? "bg-primary/10 text-primary font-medium" : "text-muted-foreground hover:bg-accent hover:text-accent-foreground"}`}
                  >
            ...
            {bottomNavItems.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`flex items-center px-2 py-2 rounded-md transition-colors ${isActive ? "bg-primary/10 text-primary font-medium" : "text-muted-foreground hover:bg-accent hover:text-accent-foreground"}`}
                >
                  <Icon className="h-5 w-5 shrink-0 md:mr-3" />
                  <span className="hidden md:inline">{item.label}</span>
                </Link>
